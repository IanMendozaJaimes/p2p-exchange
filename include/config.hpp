...
#define DEFINE_CONFIG_GET_UINT64 \
...
#define DEFINE_CONFIG_GET_INT64 \
...
#define DEFINE_CONFIG_GET_DOUBLE \
...
#define DEFINE_CONFIG_GET_ASSET \
...
#define DEFINE_CONFIG_GET_STRING \
      asset config_get_string (name key) { \
            auto citr = config.find(key.value);\
            if (citr == config.end()) { \
                  eosio::check(false, ("settings: the "+key.to_string()+" parameter has not been initialized").c_str());\
            }\
